...
@DATASET_BUILDER_REGISTRY.register('resfit_lerobot_dataset_factory')
class ResfitLerobotDatasetFactory:
    def build(self, opt_params: dict[str, Any] | None, params) -> dict[str, Any]:
        ...
        # Actions: from 0 (current) up to (horizon-1) steps in the future
        action_timestamps = [i * dt for i in range(action_horizon)]
        reward_timestamps = [i * dt for i in range(reward_horizon)]

        # Observations: from 0 (current) to -(history-1) steps in the past
        obs_proprio_timestamps = [i * dt for i in range(action_horizon - 1, -1, -3)]
        obs_images_timestamps = [i * dt for i in range(action_horizon - 1, -1, -3)]
